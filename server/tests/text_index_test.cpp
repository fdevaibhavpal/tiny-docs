...
TEST(TextIndex, ExtractsTextSpans) {
    TempPdfFile pdf("tiny_docs_text_index.pdf", makeTextPdf());
    ...
    TextIndex index;
    const auto spans = index.getSpans(document, 1);
    ...
    for (const auto& span : spans) {
        ...
        if (!span.text.empty()) {
            EXPECT_GT(span.width, 0.0);
            EXPECT_GT(span.height, 0.0);
        }
    }
    EXPECT_NE(combinedText.find("HELLO"), std::string::npos);
    EXPECT_NE(combinedText.find("TEXTINDEX"), std::string::npos);
}

TEST(TextIndex, RejectsBadPageNumber) {
    TempPdfFile pdf("tiny_docs_text_index_bad_page.pdf", makeTextPdf());
    DocumentManager manager;
    ...
    TextIndex index;
    EXPECT_THROW(index.getSpans(document, 2), std::out_of_range);
}
